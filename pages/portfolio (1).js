...
export default function Portfolio() {
  ...
  const exportJSON = () => {
    ...
    const data = JSON.stringify({ version: 1, positions: holdings, exported: new Date().toISOString() }, null, 2);
  ...
  const importJSON = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (holdings.length && !confirm(`Replace current portfolio (${holdings.length}) with imported (${positions.length})?`)) return;
        const clean = positions.map(p => ({
          id: p.id || ('p' + Date.now() + Math.random()),
          tkr: p.tkr || '', type: p.type || 'stock',
          qty: parseFloat(p.qty) || 0,
          price: parseFloat(p.price) || 0,
          chg: parseFloat(p.chg) || 0,
        })).filter(p => p.tkr && p.qty > 0 && p.price > 0);
        setHoldings(clean);
        showToast(`Imported ${clean.length} positions`);
  ...
  return (
    <>
        ...
        <div className="page-inner">
          ...
          <section className="add-card">
            ...
            <p className="add-hint">
              For <b>bonds</b>: quantity = face value, price = clean price per 100. For <b>stocks</b> and <b>ETFs</b>: shares × price. For <b>FX</b>: cash amount × rate to USD.
            </p>
          </section>
      ...
      <style jsx>{`
        ...
        .add-row { display: grid; grid-template-columns: 1.4fr 1fr 1fr 1fr 0.9fr auto; gap: 14px; align-items: end; }
        ...
        .btn-add { padding: 11px 22px; background: var(--ink); color: var(--paper); font-family: var(--sans); font-weight: 500; font-size: 13px; letter-spacing: .02em; white-space: nowrap; transition: background .15s; }
        .btn-add:hover { background: var(--accent); }
        .add-hint { margin-top: 14px; font-family: var(--sans); font-size: 12.5px; color: var(--ink-3); font-style: italic; line-height: 1.5; padding-top: 14px; border-top: 1px solid var(--rule); }
        ...
        @media (max-width: 960px) {
          ...
          .add-row .btn-add { grid-column: span 2; padding: 12px; }
